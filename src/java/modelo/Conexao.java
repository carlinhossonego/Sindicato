...
//Método de conexão
    private Connection conectar() {
        ...
        try {
            ...
            con = DriverManager.getConnection("jdbc:mysql://" + IP2 + "/Sindicato?useSSL=false", "carlim", "carlim");
            return con;
    ...
    public ArrayList<JavaBeansRelatorios> relatorio(JavaBeansRelatorios relatorios) {
        ...
        /**
         * String read = "SELECT Cortes.*,Associado.nome as
         * nome_associado,Dependentes.nome from Cortes\n" + "inner join
         * Associado on Associado.id = Cortes.associado_id\n" + "inner join
         * Dependentes on Dependentes.id = Cortes.associado_iddependente\n" +
         * "where Cortes.id_empresa = " + id + "";*
         */
        try {
            Connection con = conectar();
    ...
    public void validacao(JavaBeans2 user) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                // Move to beginning
                if (rows == 1) {
                    user.setLogin_user_sql(rs.getString(2));
                    user.setSenha_user_sql(rs.getString(3));
                    user.setNome_user_sql(rs.getString(4));
                    user.setNome_menu(rs.getString(4));
                    //JOptionPane.showMessageDialog(null, user.getNome_menu());
                }
            } else {
                user.setLogin_user_sql("");
                user.setSenha_user_sql("");
                user.setNome_user_sql("");
                user.setNome_menu("");
                //JOptionPane.showMessageDialog(null, user.getNome_user_sql());
            }
    ...
    public void validacao3(JavaBeansAssociados associados) {
        String read = "select * from Associado where login = ? and senha = ?";

        try {
            Connection con = conectar();
            PreparedStatement pst = con.prepareStatement(read);
            pst.setString(1, associados.getLogin_associado());
            pst.setString(2, associados.getSenha_associado());
            ResultSet rs = pst.executeQuery();
            ...
            if (rs.next()) {
                ...
                // Move to beginning
                if (rows == 1) {
                    associados.setLogin_associado_sql(rs.getString(10));
                    associados.setSenha_associado_sql(rs.getString(11));

                }
            } else {
                associados.setLogin_associado_sql("");
                associados.setSenha_associado_sql("");
    ...
    public void inserirAssociado(JavaBeansAssociados associado) {
        ...
        String create = "insert into Associado (id,nome,endereco,telefone,empresa,cidade,validade,dependentes,cargo,login,senha) values (?,?,?,?,?,?,?,?,?,?,?)";
        try {
            ...
            pst.setString(10, associado.getLogin_associado());
            pst.setString(11, associado.getSenha_associado());
    ...
    public void selecionarAssociado(JavaBeansAssociados associado) {
        ...
        String read2 = "select Associado.id,Associado.nome,Associado.endereco,Associado.telefone,Associado.empresa,\n"
                + " Associado.cidade, date_format(str_to_date(Associado.validade, '%Y-%m-%d'), '%d/%m/%Y')as validade, Associado.dependentes,Associado.cargo,Associado.login,Associado.senha from Associado where id = ?";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                associado.setLogin_associado(rs.getString(10));
                associado.setSenha_associado(rs.getString(11));
            }
            con.close();
        } catch (Exception e) {
            //JOptionPane.showMessageDialog(null, e);
    ...
    public void alterarAssociado(JavaBeansAssociados associado) {
        String create = "update Associado set nome=?, endereco=?, telefone=?, empresa=?, cidade=?, validade=?, dependentes=?, cargo=?, login=?, senha=? where id=?";
        ...
        try {
            ...
            pst.setString(9, associado.getLogin_associado());
            pst.setString(10, associado.getSenha_associado());

            //O ID fica por ULTIMO
            pst.setString(11, associado.getId_associado());
    ...
    public ArrayList<JavaBeansAssociados> listarDependentes(JavaBeansAssociados associado) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                //colocando no Arraylist
                listarDependentes.add(new JavaBeansAssociados(iddependentes, id_do_associano, nome, nascimento, id2, id3));
            }
            con.close();
    ...
    public ArrayList<JavaBeans2> listarUserPesquisa(JavaBeans2 pesquisa) {
        String p;
        p = pesquisa.getPesquisa();
        //JOptionPane.showMessageDialog(null, p);

        //Criar objeto para acessar a classe JavaBeans
        ArrayList<JavaBeans2> user = new ArrayList<>();

        String read = "select id, login, senha, nome from Usuario where nome like '%" + p + "%'order by nome";
        try {
            Connection con = conectar();
            PreparedStatement pst = con.prepareStatement(read);
            ResultSet rs = pst.executeQuery();
